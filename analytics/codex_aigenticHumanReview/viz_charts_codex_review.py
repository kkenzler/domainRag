...
def score_bars(ax, by_cond: dict):
    conds = list(by_cond.keys())
    metrics = ["review_source_alignment", "review_distractor_quality", "review_stem_clarity"]
...
def decision_heatmap(ax, items: list):
    ...
    for i, cond in enumerate(conds):
        for j, diff in enumerate(diffs):
            ...
            matrix[i, j] = (sum(1 for it in sub if it["review_decision"] == "ACCEPT") / len(sub) * 100) if sub else 0
...
def reject_breakdown(ax, by_cond: dict):
    ...
    for j, (diff, col) in enumerate(zip(diffs, dcols)):
        counts = [sum(1 for it in by_cond[c] if it["review_decision"] == "REJECT" and it["difficulty"] == diff) for c in conds]
...
def radar_by_condition(ax, by_cond: dict):
    metrics = [
        ("review_source_alignment", "Alignment\n(1–5)"),
        ("review_distractor_quality", "Distractor\n(1–5)"),
        ("review_stem_clarity", "Clarity\n(1–5)"),
    ]
    conds = list(by_cond.keys())
...
def accept_vs_match(ax, by_cond: dict):
    ...
    x = np.arange(len(conds))
    w = 0.32
    accept_rates = [sum(1 for it in by_cond[c] if it["review_decision"] == "ACCEPT") / len(by_cond[c]) * 100 for c in conds]
    match_rates = []
    for c in conds:
        matched = 0
        for it in by_cond[c]:
            val = it.get("review_difficulty_match")
            if val is True:
                matched += 1
